...
class TransactionExporter extends Exporter
{
    ...
    public static function getCompletedNotification(Export $export): Notification
    {
        return Notification::make()
            ->title('Transaction export completed')
            ->success()
            ->body('Your export has finished and ' . number_format($export->successful_rows) . ' rows have been exported.')
            ->actions([
                \Filament\Notifications\Actions\Action::make('download')
                    ->label('Download')
                    ->url(route('filament.admin.exports.download', ['export' => $export]))
                    ->button(),
            ]);
    }
}
